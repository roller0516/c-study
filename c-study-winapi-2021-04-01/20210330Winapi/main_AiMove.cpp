...
void UpdateAiPlayer()
{
	if (player[Controller::_2P].direction == Direction::Up)
	{
		// 위로 이동하는 코드
		player[Controller::_2P].pos.y -= 10;

		if (player[Controller::_2P].pos.y < 0)
		{
			player[Controller::_2P].direction = Direction::Down;
		}
	}
	else
	{
		...
		if (player[Controller::_2P].pos.y > 720)
		{
			player[Controller::_2P].direction = Direction::Up;
		}
	}

	player[Controller::_2P].shape =
		GetRectToCenter(player[Controller::_2P].pos.x,
			player[Controller::_2P].pos.y, 100, 100);
...
{
	// 1P
	player[Controller::_1P].pos = { 100, 300 };
	player[Controller::_1P].shape = GetRectToCenter(
		player[Controller::_1P].pos.x, player[Controller::_1P].pos.y, 100, 100);
	player[Controller::_1P].direction = Direction::Up;
	// 2P
	player[Controller::_2P].pos = { 800, 300 };
	player[Controller::_2P].shape = GetRectToCenter(
		player[Controller::_2P].pos.x, player[Controller::_2P].pos.y, 100, 100);
	player[Controller::_2P].direction = Direction::Up;
}
...
void RenderPlayer(HDC hdc)
{
	for (int i = 0; i < Controller::END; i++)
	{
		Rectangle(hdc, player[i].shape.left, player[i].shape.top,
			player[i].shape.right, player[i].shape.bottom);
	}
}
