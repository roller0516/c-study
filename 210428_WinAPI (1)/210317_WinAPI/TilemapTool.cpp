...
HRESULT TilemapTool::Init()
{
    ...
    // UI Button
    ImageManager::GetSingleton()->AddImage("저장버튼", "Image/SaveImage.bmp",
        1282/10, 726/10, 1, 2);
    ImageManager::GetSingleton()->AddImage("불러오기버튼", "Image/LoadImage.bmp",
        1286/10, 724/10, 1, 2);
    ImageManager::GetSingleton()->AddImage("다음버튼", "Image/SaveImage.bmp",
        1282 / 10, 726 / 10, 1, 2);
    ImageManager::GetSingleton()->AddImage("이전버튼", "Image/LoadImage.bmp",
        1286 / 10, 724 / 10, 1, 2);

    btnSave = new Button();
    btnSave->Init("저장버튼", 750,
       200);
     btnSave->SetFunc(Save, count);

    btnLoad = new Button();
    btnLoad->Init("불러오기버튼", 750,
        300);
    btnLoad->SetFunc(Load, count);

    btnNext = new Button();
    btnNext->Init("다음버튼",800,
        500);
    btnNext->SetFunc(StageUp);
    btnPrev = new Button();
    btnPrev->Init("이전버튼", 1000,
        500);
