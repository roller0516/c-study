...
void DuoLon::Render(HDC hdc)
{
	...
	if(comboAttack == true)
		TextOut(hdc, 20, 50, "콤보어택", strlen("콤보어택"));
}
...
void DuoLon::ComboAttack()
{
	if (KeyManager::Getsingleton()->IsOnceKeyDown(VK_DOWN))  //대각선 오른쪽 A버튼
	{

		if (KeyManager::Getsingleton()->IsOnceKeyDown('A')) 
		{
			if (KeyManager::Getsingleton()->GetKeyValue()[1] == 'A')
				comboAttack == true;
		}	
		/*key[count] = VK_DOWN;
		count++;
		if (key[count] == VK_DOWN)
		{
			count++;
		...
		//{
		//	if (inPutKey[i] == VK_DOWN)
		//	{
		//		i++;
		//		if (inPutKey[i] == VK_RIGHT)
		//		{
		//			i++;
		//			if (inPutKey[i] == 'A')
		//			{
